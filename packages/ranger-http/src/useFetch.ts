...
function useFetch<TData = any>(...args: UseFetchArgs): UseFetch<TData> {
  ...
  const makeFetch = useDeepCallback(
    (method: HTTPMethod): FetchData<TData> => {
      const doFetch = async (routeOrBody?: RouteOrBody, body?: UFBody): Promise<any> => {
        ...
        try {
          ...
          res.current = interceptors.response
            ? await interceptors.response({ response: res.current, request: requestInit })
            : res.current
          invariant(
            'data' in res.current,
          ...
          const opts: any = { attempt: attempt.current, error: err }
          const shouldRetry =
            // if we just have `retries` set with NO `retryOn` then
            // automatically retry on fail until attempts run out
      ...
      const retry = async (opts: RetryOpts, routeOrBody?: RouteOrBody, body?: UFBody) => {
        const delay = (
          isFunction(retryDelay) ? (retryDelay as Function)(opts) : retryDelay
        ) as number
        if (!(Number.isInteger(delay) && delay >= 0)) {
          console.error(
            'retryDelay must be a number >= 0! If you are using it as a function, it must also return a number >= 0.'
          )
        }
        attempt.current++
  ...
  const response: any = useMemo(() => toResponseObject<TData>(res, data), [])
