...
export const useExampleURL = (): string => {
  ...
  return useMemo(
    (): string => (isBrowser ? (window.location.origin as string) : 'https://example.com'),
    [isBrowser]
  )
}
...
export function useURLRequiredInvariant(
  ...
  useEffect((): void => {
    invariant(
      condition,
      `${method} requires a URL to be set as the 1st argument, unless you wrap your app like: <Provider url="${exampleURL}"><App /></Provider> ${optionalMessage}`
    )
