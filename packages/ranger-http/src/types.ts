...
export type UseFetchArrayReturn<TData> = [Req<TData>, Res<TData>, boolean, Error]
...
export type UseFetch<TData> = UseFetchArrayReturn<TData> & UseFetchObjectReturn<TData>
...
export type RetryOn =
  ...
  (<TData = any>({ attempt, error, response }: RetryOpts) => Promise<boolean>) | number[]
...
export type Flatten<T> = Pick<T, NonObjectKeysOf<T>> & UnionToIntersection<ObjectValuesOf<any>>
