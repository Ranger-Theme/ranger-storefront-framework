...
export function renderPrice(
  ...
  if (product.price) {
    ...
    if (regular.amount.value === final.amount.value) {
      return html`
        <span class="price-final">${format(final.amount.value)}</span>
      `
    }
  ...
  if (product.priceRange) {
      ...
      return html`
        <div class="price-range">
          ${finalMin.amount.value !== regularMin.amount.value
            ? html`
                <span class="price-regular">${format(regularMin.amount.value)}</span>
              `
            : ''}
          <span class="price-from">
            ${format(finalMin.amount.value)} - ${format(finalMax.amount.value)}
          </span>
        </div>
      `
    }
    ...
    return html`
      <span class="price-final">${format(finalMin.amount.value)}</span>
    `
  }
